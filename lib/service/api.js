...
class Api {
  constructor() {
	...
	router.get('/api/chiastats', this.onGetChiaStats.bind(this));
  ...
  async onGetChiaStats(ctx) {
    const ChiaStats = await APIServices.getStats();
    ctx.body = ChiaStats;
  ...
  async onUpdateSatellite(ctx) {
    ...
    if (body.collapsed !== undefined && satellite.collapsed !== body.collapsed) {
